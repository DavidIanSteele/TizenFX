...
using Camera = Tizen.NUI.Scene3D.Camera;
...
class Physics3DDemo : NUIApplication
{
    ...
    private static string IMAGE_DIR = Tizen.Applications.Application.Current.DirectoryInfo.Resource + "images/";
    private static string BALL_TEXTURE_URL = IMAGE_DIR + "/background-3.jpg";
    ...
    protected override void OnCreate()
    {
        ...
        _window.BackgroundColor = Color.HotPink;
        _windowSize = _window.WindowSize;
        _window.KeyEvent += OnKeyEvent;

        var camera = new Camera();
        camera.FieldOfView = new Radian(MathF.PI * 0.666f);
        camera.NearPlaneDistance = 0.1f;
        camera.FarPlaneDistance = 1000.0f;
        camera.ProjectionMode = Camera.ProjectionModeType.Perspective;
        ...
        {
            Size = new Size(_windowSize.Width, _windowSize.Height),
            ParentOrigin = ParentOrigin.TopLeft,
            PivotPoint = PivotPoint.TopLeft,
            BackgroundColor = Color.Beige,
            PositionUsesPivotPoint = true
        };
        _window.Add(sceneView);
        
        _physicsAdaptor.SetRootView(sceneView);
    ...
    private void CreateWalls(DiscreteDynamicsWorld dynamicsWorld)
    {
        var size = new Vector3(2.0f * _windowSize.Width, 10.0f, 2.0f * _windowSize.Height);
        var groundView = new View();
    ...
    private void CreateBall(DiscreteDynamicsWorld dynamicsWorld)
    {
        ...
        var ballRenderer = new BallRenderer(new Vec3(2.0f*ballRadius, 2.0f*ballRadius, 2.0f*ballRadius), BALL_TEXTURE_URL);
        var physicsBall = _physicsAdaptor.AddViewToBody(ballRenderer.View, body);
        _renderers.Add(ballRenderer);
